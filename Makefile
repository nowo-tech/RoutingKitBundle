# Makefile for Routing Kit Bundle
...
.PHONY: help up down down-dev build shell install ensure-up test test-coverage coverage-php-percent cs-check cs-fix qa clean release-check release-check-demos demo-smoke composer-sync rector rector-dry phpstan update validate assets setup-hooks check-no-cursor-coauthor check-open-prs strip-cursor-coauthor-from-history validate-translations coverage-check check-twig-extra

help:
	@echo "Routing Kit Bundle - Development Commands"
	@echo ""
	...
	@echo "  release-check Pre-release: open PRs, Cursor trailer, cs, phpstan, coverage, demos"
...
check-open-prs:
	...
	@GH_REPO=nowo-tech/RoutingKitBundle ./.scripts/check-open-prs.sh
